...
def create_data():
    ...
    cursor.execute(sql, ("0", "1", "0", "0", "성냥팔이 소녀가 성냥을 한 상자도 팔지 못했을 때 기분이 어땠을까요?", "두려,고려,무서,무섭,걱정"))
    cursor.execute(sql, ("0", "1", "0", "1", "왜 그렇게 생각했나요?", "아빠,아버지,화,분노,두려,무서,무섭"))
    cursor.execute(sql, ("0", "1", "1", "0", "소녀가 성냥에 불을 붙여 따뜻한 음식들과 난로를 보았을 때 기분이 어땠을까요?", "행복,기뻤,꿈,깁,집,지"))
    cursor.execute(sql, ("0", "1", "1", "1", "왜 그렇게 생각했나요?", "원했,갖고,희망,소망"))
    cursor.execute(sql, ("0", "1", "2", "0", "소녀가 할머니를 봤을 때 기분이 어땠을까요?", "행복,기뻤,꿈,깁,집,지"))
    cursor.execute(sql, ("0", "1", "2", "1", "왜 그렇게 생각했나요?", "그리워,보고싶"))
    cursor.execute(sql, ("0", "1", "3", "0", "길에서 성냥팔이 소녀를 발견한 사람들의 기분은 어땠을까요?", "안타까,불쌍,가엾,가여,됐다,없,엽"))
    cursor.execute(sql, ("0", "1", "3", "1", "왜 그렇게 생각했을까요?", "외로,외롭,혼자,안타까,불쌍,누워,누웠"))
